...
|-----------------------------|--------------------------------------------------------|---------------------------------------------------------------------------------------------------|
| **mobile < 768**            | Full-width row above logs. **Title hidden** (no "Hardware Utilization" label — viewport too narrow to carry it and the chips). Live dot + 3 chips + chevron inline using the shared chrome (20px chip rhythm, 6×6 dot, chevron pulled right via `margin-left:auto`). Spacer reserves vertical room above logs. | Same width, grows downward into logs (overlay) |
