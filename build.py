...
(function () {
  ...
  window.hwToggleV4 = function (ev) {
    ...
    // Commit the target state + content.
    v4Expanded = willExpand;
    pill.classList.toggle('hw-v4-expanded', willExpand);
    pill.classList.toggle('hw-v4-collapsed', !willExpand);
    applyState(currentState);

    // Fallback: no GSAP → instant (original behaviour).
    if (typeof gsap === 'undefined') return;

    if (v4Tl) v4Tl.kill();   // cancel any in-flight animation (fast toggles)
    var startH = pill.offsetHeight;            // current (animated) height
    gsap.set(pill, { height: 'auto' });
    var endH = pill.offsetHeight;              // natural target height
    gsap.set(pill, { height: startH, overflow: 'hidden' });
    v4Tl = gsap.timeline({
      onComplete: function () { gsap.set(pill, { clearProps: 'height,overflow' }); }
    });
    v4Tl.to(pill, { height: endH, duration: 0.4, ease: 'power3.inOut' }, 0);
  };
  function attachV4() {
    var pill = document.getElementById('hw-v4-pill');
